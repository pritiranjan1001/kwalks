...
$user_id = true;
$u=$_SESSION['user_id'];
$query = mysql_query(" SELECT id FROM puser where id=$u and ver=0");
...
$check_login=mysql_query("SELECT ver_code,act FROM puser WHERE  id=('$u_id')");
$get = mysql_fetch_array($check_login);
$num=$get['ver_code'];
$act=$get['act'];
if($verify==$num)
{
mysql_query("UPDATE puser SET ver='1'WHERE  id=('$u_id')");
if ($act=='w') {
