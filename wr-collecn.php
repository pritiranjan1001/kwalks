...
<?php
if(isset($_GET['u_id'])&& !empty($_GET['u_id'])){
}else{$u_id=$_SESSION['user_id'];
echo" <a href='wr-note.php'><i class='fa fa-pencil-square-o'></i></a>";?><hr />
<?php } ?> 
</div>
</div>
...
<?php     
if(isset($_GET['u_id'])&& !empty($_GET['u_id'])){
$query = mysql_query(" SELECT id,name,descptn,url,upload_date FROM w_notes  where user_id=('$u_id') ORDER BY id DESC");  
}else{
$query = mysql_query(" SELECT id,name,descptn,url,upload_date FROM w_notes  where  user_id=('$user') ORDER BY id DESC"); 
}                                            
...
<div class="centered">
<h4>  <?php echo $name; ?></h4><hr>
</div>
<div class="light-pricing">
<p>
<?php if($pic !='nothing'){?>
<img img src="uploades/w-notes/<?php echo $pic ;?>" class="img-responsive"><?php } ?>
